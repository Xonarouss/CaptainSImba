...
def code_to_icon(code: int):
    return WEATHER_CODE.get(code, ("🌡️", "Weather"))
...
class Weather(commands.Cog):
    ...
    @app_commands.command(name="weather", description="Show current weather and 7-day forecast.")
    @app_commands.describe(location="City / place", unit="c or f")
    async def weather(self, interaction: discord.Interaction, location: str, unit: str = "c"):
        ...
        title = f"{icon} Weather — {name}{', ' + country if country else ''}"
